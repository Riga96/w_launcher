...
/**
 * Whether a bookmark title is empty or the auto-generated default.
 * @param {string} title
 * @param {string} workId
 * @returns {boolean}
 */
export function isAutoGeneratedTitle(title, workId) {
  const trimmed = (title || '').trim();
  if (!trimmed) return true;
  return trimmed === defaultWorkTitle(workId);
}

/**
 * Try to parse iPhone Shortcut clipboard JSON.
 * @param {string} text
 * @returns {{ url: string, title: string } | null}
 */
export function parseShortcutClipboardJson(text) {
  const trimmed = (text || '').trim();
  if (!trimmed.startsWith('{')) return null;

  try {
    const obj = JSON.parse(trimmed);
    const url = String(obj.url || obj.URL || '').trim();
    if (!url) return null;
...
/**
 * Whether an imported title should replace the stored bookmark title.
 * @param {string} currentTitle
 * @returns {boolean}
 */
export function shouldReplaceImportedTitle(currentTitle) {
  const trimmed = (currentTitle || '').trim();
  if (!trimmed) return true;
  return trimmed.startsWith('webtoons-');
}

/**
 * Strip site suffix starting at " BlackToon".
 * @param {string} title
 * @returns {string}
 */
function stripSiteTitleSuffix(title) {
  const text = (title || '').trim();
  if (!text) return '';

  const idx = text.search(/\s+BlackToon/i);
  if (idx === -1) return text;
  return text.slice(0, idx).trim();
}

/**
 * Parse a Shortcut/Safari page title into work title and display episode.
 * @param {string} rawTitle
 * @returns {{ workTitle: string, displayEpisode: string }}
 */
export function parseShortcutPageTitle(rawTitle) {
  const text = stripSiteTitleSuffix(rawTitle);
  if (!text) return { workTitle: '', displayEpisode: '' };

  const matches = [...text.matchAll(/(\d+화)/g)];
  if (!matches.length) {
    return { workTitle: text.trim(), displayEpisode: '' };
  }

  const displayEpisode = matches[matches.length - 1][1];
  const lastIndex = text.lastIndexOf(displayEpisode);
  const workTitle = (
    text.slice(0, lastIndex) + text.slice(lastIndex + displayEpisode.length)
  ).trim();

  return { workTitle, displayEpisode };
}

/**
 * Remove episode suffixes from an imported page title.
 * @param {string} title
 * @returns {string}
 */
export function sanitizeImportedTitle(title) {
  return parseShortcutPageTitle(title).workTitle;
}
